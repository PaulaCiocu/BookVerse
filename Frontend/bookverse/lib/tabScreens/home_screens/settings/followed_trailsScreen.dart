...
class _FollowedTrailsscreenState extends State<FollowedTrailsscreen> {
  ...
  Future<void> fetchTrails() async {
    ...
    try {
      final response = await http.get(Uri.parse('http://10.0.2.2:8080/reading-trails/followed/person/${widget.userId}'));
  ...
  // Function to delete a trail from reading list
  Future<bool> unfollowTrailFromReadingList(String trailId, bool keepBooksConfirmed) async {
    final url = Uri.parse('http://10.0.2.2:8080/reading-trails/delete/${widget.userId}/$trailId/$keepBooksConfirmed');
                                                      ...
                                                      onTap: () async {
                                                        ...
                                                        if (unfollowConfirmed == true) {
                                                          ...
                                                          bool? keepBooksConfirmed = await showDialog<bool>(
                                                            ...
                                                            unfollowTrailFromReadingList(trail['trail']['id'].toString(), keepBooksConfirmed!);
